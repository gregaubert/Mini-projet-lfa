...
## Author:        L. Markowsky
## Adapter:       C.-A. Peña-Reyes
## Keywords:      fuzzy-logic-toolkit fuzzy rule interpretable-fuzzy-systems
## Directory:     ifs_functions
## Filename:      ifs_showrule.m
## Last-Modified: 17 Apr 2012

function ifs_showrule (fis, index_list = [], format = 'verbose', ...
                   language = 'english', ...
  ...
  if (!(nargin >= 1 && nargin <= 5))
    puts ("Type 'help ifs_showrule' for more information.\n");
...
function mf_name = get_mf_name (mem_fcn_value, fis_input_or_output)
  ...
  mf_name = fis_input_or_output.mf_names(abs(fix(mem_fcn_value)),:);
  mf_name = deblank(mf_name);
...
function showrule_verbose_format (fis, index_list, language, ...
  ...
  for i = 1 : length (index_list)
    ...
    ##     " (<weight>)"
    ## IFS SYSTEMS DON'T USE WEIGHTS FOR RULES SO THIS PART IS NOT REQUIRED
    ##------------------------------------------------------------------
    ...
    ## Default Rule
    ## Print the consequent in the form:
    ##     ", else (output-name is [hedge] mem-fcn-name) and
    ##             (output-name is [hedge] mem-fcn-name) and 
    ##             ...
    ##             (output-name is [hedge] mem-fcn-name)"
    ##
...
if isfield(fis, 'defrule')
   if not(isempty(fis.defrule)),
    ...
    printf("%s","D.")
    current_con = fis.defrule.consequent;
    first_output_printed = true;
    for j = 1 : num_outputs
