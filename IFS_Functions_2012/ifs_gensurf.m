...
## ifs_gensurf is a minor adaptation of gensurf for the ifs (Interpretable Fuzzy Systems) toolkit by Carlos Andrés Peña

function [x, y, z] = ifs_gensurf (ifs, input_axes = [1 2], output_axis = 1, ...
                              grids = [15 15], ref_input = [])
...
#  elseif (!is_ifs (ifs))
#    puts ("Type 'help gensurf' for more information.\n");
#    error ("gensurf's first argument must be an ifs structure\n");
#  elseif ((nargin >= 2) && !are_input_indices (input_axes, ifs))
#    puts ("Type 'help gensurf' for more information.\n");
#    error ("gensurf's second argument must be valid input indices\n");
#  elseif ((nargin >= 3) && !is_output_index (output_axis, ifs))
#    puts ("Type 'help gensurf' for more information.\n");
...
#  elseif ((nargin == 5) && !is_ref_input (ref_input, ifs, input_axes))
#    puts ("Type 'help gensurf' for more information.\n");
#    error ("gensurf's fifth argument must specify reference input values\n");
  endif

  if (length (input_axes) == 1 || columns (ifs.input) == 1)
    generate_plot (ifs, input_axes, output_axis, grids, ref_input);
  else
    generate_surface (ifs, input_axes, output_axis, grids, ref_input);
  endif
...
## Purpose:  Generate a plot representing one of the ifs outputs as a
##           function of one of the ifs inputs.
##------------------------------------------------------------------------------

function [x, y, z] = generate_plot (ifs, input_axis, output_axis, grids, ...
                                    ref_input, num_points)

  ## Create input to ifs using grid points and reference values.

  num_inputs = columns (ifs.input);
  num_grid_pts = grids(1);
  ifs_input = zeros (num_grid_pts, num_inputs);
  ...
  for i = 1 : num_inputs
    if (i == input_axis)
      x_axis = (linspace (ifs.input(i).range(1), ...
                          ifs.input(i).range(2), ...
                          num_grid_pts))';
      ifs_input(:, i) = x_axis;
    else
      ifs_input(:, i) = ref_input(i) * ones (num_grid_pts, 1);
    endif
  endfor

  ## Compute and plot the output.

  output = ifseval (ifs_input, ifs);
  figure ('NumberTitle', 'off', 'Name', ifs.name);
  plot (x_axis, output, 'LineWidth', 2);
  xlabel (ifs.input(input_axis).name, 'FontWeight', 'bold');
  ylabel (ifs.output(output_axis).name, 'FontWeight', 'bold');
  grid;
  hold;
...
## Purpose:  Generate a surface representing one of the ifs outputs as a
##           function of two of the ifs inputs.
##------------------------------------------------------------------------------

function [x, y, z] = generate_surface (ifs, input_axes, output_axis, grids, ...
                                       ref_input)
  ...
  for i = 1 : num_inputs
    ...
    elseif (i == input_axes(2))
      y_axis = (linspace (ifs.input(i).range(1), ...
                          ifs.input(i).range(2), ...
                          grids(2)))';
    else
      ifs_input(:, i) = ref_input(i) * ones (num_grid_pts, 1);
  ...
  ifs_input(:, input_axes(1)) = xx(:);
  ifs_input(:, input_axes(2)) = yy(:);
  ...
  xlabel (ifs.input(input_axes(1)).name);
  ylabel (ifs.input(input_axes(2)).name);
  zlabel (ifs.output(output_axis).name);
